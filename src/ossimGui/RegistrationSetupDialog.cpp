...
namespace ossimGui
{
   namespace
   {
      bool registrationTieGeneratorAvailable(const std::string& method)
      {
         return static_cast<bool>(
            ossim_autoreg::TiePointGeneratorFactory::instance()->create(
               method));
      }
      ...
      const std::string& matchMethod)
   {
      ...
      result.bundleAnchorInputIndexes =
         approach == REGISTRATION_SETUP_BUNDLE_ANCHORED
            ? std::vector<ossim_uint32>(1, 0)
            : std::vector<ossim_uint32>();
      result.bundleInputPairs.clear();
      result.bundleLinearSolverType = "auto";
         ...
         result.matchMethod.empty())
      {
        ...
        result.resamplerType = defaults.generator().resamplerType();
         ...
         result.bundleLinearSolverType =
            defaults.optimizer().bundleLinearSolverType();
         return result;
      }
